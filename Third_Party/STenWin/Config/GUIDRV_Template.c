...
*/
void dwr(int x, int y, uint16_t c)
{
	*(__IO uint16_t*) (0xD0000000 + 2*(y*LCD_PIXEL_WIDTH+x)) = c;
	
	//LCD_DrawLineX(x,y,1,0,c);
}
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, unsigned long PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
		//	*(__IO uint16_t*) (LCD_FRAME_BUFFER + y*LCD_PIXEL_WIDTH*2+x) = PixelIndex;
			//*(__IO uint16_t*) (0xD0000000 + 2*(y*LCD_PIXEL_WIDTH+x)) = (uint16_t)PixelIndex;
			dwr(x,y,PixelIndex);
    }
...
static unsigned long _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
      PixelIndex = *(__IO uint16_t*) (0xD0000000 + 2*(y*LCD_PIXEL_WIDTH+x));
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  PixelIndex = LCD__GetColorIndex();
	LCD_DrawFullRectX(x0,y0,x1-x0+1,y1-y0+1,PixelIndex);
}
...
static void _DrawHLine(GUI_DEVICE * pDevice, int x0, int y, int x1) {
  LCD_PIXELINDEX PixelIndex;
  PixelIndex = LCD__GetColorIndex();	
	LCD_DrawLineX(x0,y,x1-x0+1,LCD_DIR_HORIZONTAL,PixelIndex);
}
...
static void _DrawVLine(GUI_DEVICE * pDevice, int x, int y0, int y1) {
  LCD_PIXELINDEX PixelIndex;
  PixelIndex = LCD__GetColorIndex();	
	LCD_DrawLineX(x,y0,y1-y0+1,LCD_DIR_VERTICAL,PixelIndex);
}
